...
export async function buscarPorId(req: Request, res: Response): Promise<void> {
  try {
    ...
    if (!partida) res.status(404).json({ mensagem: 'Partida não encontrada' });
...
export async function atualizar(req: Request, res: Response): Promise<void> {
  try {
    ...
    if (!atualizado) res.status(404).json({ mensagem: 'Partida não encontrada para atualizar' });
...
export async function deletar(req: Request, res: Response): Promise<void> {
  try {
    ...
    if (!deletado) res.status(404).json({ mensagem: 'Partida não encontrada para deletar' });
    res.status(204).send();
  } catch (error) {
